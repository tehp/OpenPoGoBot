...
def action_catch_pokemon(event, *args, **kwargs):
    pass
...
def action_fort_found(event, fort_name="Unknown", fort_distance=0.0, **kwargs):
    logger.log(u"[#] Found fort {} at distance {}".format(fort_name, fort_distance))


@manager.on("fort_moving")
def action_fort_moving(event, fort_name="Unknown", **kwargs):
    logger.log(u"[#] Moving closer to {}".format(fort_name))


@manager.on("fort_arrived")
def action_fort_arrived(event, fort_name="Unknown", **kwargs):
